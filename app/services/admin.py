...
def create_user(
    ...
    db.flush()
    write_audit_log(
        db, "USER_CREATE", "USER", str(user.id), admin_user_id,
...
def reset_password(db: Session, user_id: int, new_password: str, admin_user_id: int) -> None:
    user = db.scalar(select(User).where(User.id == user_id))
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")

    user.password_hash = hash_password(new_password)
    write_audit_log(db, "PASSWORD_RESET", "USER", str(user.id), admin_user_id)
    db.commit()
...
def list_users(db: Session) -> list[dict]:
    rows = db.execute(
        select(User.id, User.employee_id, User.full_name, Role.name.label("role"), User.is_active)
...
def toggle_user_active(db: Session, user_id: int, admin_user_id: int) -> User:
    user = db.scalar(select(User).where(User.id == user_id))
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
    ...
    write_audit_log(
        db, "USER_TOGGLE_ACTIVE", "USER", str(user.id), admin_user_id,
